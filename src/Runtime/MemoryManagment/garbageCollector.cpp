...
/* constexpr size_t pageSize = 64 * 1024; // page size in bytes */
...
static inline void runObjDestructor(object::Obj *obj) {
  ...
  case +object::ObjType::ARRAY:
    ...
    return;
  case +object::ObjType::FILE:
    reinterpret_cast<object::ObjFile *>(obj)->~ObjFile();
    return;
  case +object::ObjType::FUTURE:
    reinterpret_cast<object::ObjFuture *>(obj)->~ObjFuture();
    return;
  case +object::ObjType::HASH_MAP:
    reinterpret_cast<object::ObjHashMap *>(obj)->~ObjHashMap();
    return;
  case +object::ObjType::MUTEX:
    reinterpret_cast<object::ObjMutex *>(obj)->~ObjMutex();
    return;
  default:
    return;
  }
...
void *GarbageCollector::alloc(const uInt64 size) {
  // No thread is marked as suspended while allocating, even though they have to
  // lock the allocMtx Every thread that enters this function is guaranteed to
  // exit it or to crash the whole program
  ...
  if (idx == -1) {
    try {
      ...
      errorHandler::addSystemError(
          fmt::format("Failed allocation, tried to allocate {} bytes", size));
    }
    // Flags that this pointer was malloc-d
    reinterpret_cast<Obj *>(block)->allocType = GCAllocType::MALLOC;
    reinterpret_cast<Obj *>(block)->GCData =
        1; // When alloc type == MALLOC GC data serves as a "marked" flag
...
void GarbageCollector::collect(std::unique_lock<std::mutex> &lk) {
#ifdef GC_DEBUG
  double d = duration_cast<std::chrono::milliseconds>(
  ...
  markRoots();
#ifdef GC_DEBUG
  double d2 = duration_cast<std::chrono::milliseconds>(
                  std::chrono::system_clock::now().time_since_epoch())
                  .count();
  ...
  resetMarkFlag();
#ifdef GC_DEBUG
  double d3 = duration_cast<std::chrono::milliseconds>(
                  std::chrono::system_clock::now().time_since_epoch())
                  .count();
  ...
  mark();
#ifdef GC_DEBUG
  double d4 = duration_cast<std::chrono::milliseconds>(
                  std::chrono::system_clock::now().time_since_epoch())
                  .count();
  ...
  sweep();
#ifdef GC_DEBUG
  double d5 = duration_cast<std::chrono::milliseconds>(
                  std::chrono::system_clock::now().time_since_epoch())
                  .count();
  std::cout << "sweeping took: " << d5 - d4 << "\n";
#endif
  while (heapSize > heapSizeLimit)
    heapSizeLimit = heapSizeLimit << 1;
...
static inline bool markIfNotMarked(object::Obj *ptr, MemoryPool *mempools) {
  ...
  if (ptr->allocType == GCAllocType::MALLOC) {
    ...
    if (!mempools[ptr->allocType].isFree(reinterpret_cast<uintptr_t>(ptr)))
      return true;
...
static inline bool isMarked(object::Obj *ptr, MemoryPool *mempools) {
  if (ptr->allocType == GCAllocType::CONSTANT)
    return true;
  if (ptr->allocType == GCAllocType::MALLOC) {
    if (ptr->GCData == 1)
      return true;
...
void GarbageCollector::mark() {
  ...
  while (!markStack.empty()) {
    ...
    markStack.pop_back();
    if (markIfNotMarked(ptr, mempools.data()))
      continue;
#ifdef GC_DEBUG
    marked++;
#endif
    ...
    case +ObjType::ARRAY: {
      ...
      // small optimization: if numOfHeapPtrs is 0 then we don't even scan the
      // array for objects and if there are objects we only scan until we find
      // all objects
      int temp = 0;
      int i = 0;
    ...
    case +ObjType::INSTANCE: {
      ...
      for (int i = 0; i < inst->fieldArrLen; i++)
        valueHelpers::mark(inst->fields[i]);
      break;
    }
    ...
    case +ObjType::FUTURE: {
      ...
      // When tracing all threads other than the main one are suspended, so
      // there's no way for anything to write to val
...
void GarbageCollector::markRoots() {
  ...
  for (auto it = threadsStack.begin(); it != threadsStack.end(); it++) {
    ...
    // The stack grows downward, so stack end is a smaller address than stack
    // start
    while (end < start) {
      // Cast pointer to int64, check for the object flag, if it's present try
      // to mark the object and push to mark stack
      Value address = *reinterpret_cast<Value *>(end);
      if (isObj(address)) {
        Obj *object = decodeObj(address);
        // There is a small chance that some random 64 bits of data on the stack
        // appear as a NaN boxed object Because of that before accessing the
        // object first we check if 'object' really points to an allocated
        // object
...
// TODO: can this be optimized? Running through every page of every mempool
// seems expensive? Should work for now but might be a problem when the heap
// gets into GB teritory
bool GarbageCollector::isAllocedByMempools(object::Obj *ptr) {
#ifdef GC_DEBUG
  for (MemoryPool &mempool : mempools) {
    if (mempool.allocedByThisPool(reinterpret_cast<uintptr_t>(ptr)))
      return true;
  }
  return false;
#else
  /*for(MemoryPool& mempool : mempools){
      if(mempool.allocedByThisPool(reinterpret_cast<uintptr_t>(ptr))) return
  true;
  }
  return false;*/
  return std::any_of(std::execution::par_unseq, mempools.begin(),
                     mempools.end(), [ptr](MemoryPool &mempool) {
                       return mempool.allocedByThisPool(
...
void GarbageCollector::sweep() {
  for (auto it = interned.begin(); it != interned.end();) {
    if (!isMarked(it->second, mempools.data()))
  ...
  std::for_each(std::execution::par_unseq, mempools.begin(), mempools.end(),
                [](MemoryPool &mempool) { mempool.resetHead(); });
#endif
}
...
void GarbageCollector::addStackStart(const std::thread::id thread,
...
void GarbageCollector::setStackEnd(const std::thread::id thread,
                                   uintptr_t *stackEnd) {
  {
    std::scoped_lock<std::mutex> lk(pauseMtx);
...
void GarbageCollector::suspendMe() {
  ...
  if (threadsStack.size() == threadsSuspended) {
    // Execute the gc cycle, we hold the lock to pauseMtx right now so no other
    // thread can alter things
    collect(lk);
    return;
  }
  // The rhs condition is because (threadsStack - 1) threads could be suspended
  // and the last thread is finishing execution In that case the last thread
  // will never enter suspendMe, but will notify a random thread waiting on
  // STWcv when it gets deleted via removeStackStart()
  STWcv.wait(lk, [&] {
    return active == 0 || threadsStack.size() == threadsSuspended;
  });
  // Wait is over, either the gc cycle is over(active == 0) or this thread has
  // been selected to run the gc cycle
  if (active == 0) {
    // All waiting that are waiting because of suspendMe need to decrement
    // threadsSuspended on their own Some threads might be waiting on a use
    // defined mutex and are thus suspended, because of that simply setting
    // threadsSuspended to 0 after a gc cycle is not possible
    threadsSuspended--;
    return;
  }
...
void GarbageCollector::addGlobalRoot(Value *ptr) { globalRoots.push_back(ptr); }
} // namespace memory
